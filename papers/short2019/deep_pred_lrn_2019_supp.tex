...
\item[Bp:] The TE layer representations from the backpropagation version of biological model, including {\em What, Where} and {\em What * Where} integration layers, trained with the V1p and V1hp (low and high resolution pulvinar) layers as final output layers, using the time $t$ target pattern from the $t-1$ input (i.e., as a predictive network).
...
% (JAR) should this be inter-cluster or intra-cluster?
...
% (JAR) Simplest is perhaps not a good adjective here. How about the most consistent results or perhaps just say, "The Bp model converged on the same cluster state regardless of starting configuration."
% (JAR) Having missed a bunch of meetings this summer I'm not fully familiar with how you did the RSA analysis and I'm not following it here. I'm confused re: "initial categories" 

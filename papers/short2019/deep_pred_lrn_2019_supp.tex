...
% (JLR) I was using "layer 3" because I started counting from 0 (not including the pixels). So there are actually 4 layers. I would just say "highest layer".
% (JLR) "standard" may not be appropriate. The number of layers we used was the same as in the original paper, but we could have very easily used a different number.
...
% (JAR) I see why I missed the centroid definition on first pass - it is defined in a caption. Suggest defining in the body as readers may look at the table but not read the full caption.
% (JAR) What we are calling "centroid" shape categories are the set that are near-best when considering both the Leabra model and the results from the human behavioral experiment (Expt).
% (JAR) What about "Base" instead of "Centroid" - I feel like centroid carries to much meaning that isn't quite right and the hill climbing algorithm should work regardless if the problem is convex. Of course if it isn't then perhaps we should be starting with more clusters and a random set.
...
All analyses in the RSA were conducted using the representations from the $R_l$. 
...
% (JLR) Should reproduce figures from Lotter et al. describing architecture?
% (JLR) Should emphasize backpropagation rather than "neural network model"?
% (JLR) Should also include a fuller description of the algorithm?
% (JLR) Should link directly to PredNet Pytorch code? And should also link to original (outdated) Tensorflow code?
% (JLR) Everyone usually cites Adam (Kingma and Ba)?

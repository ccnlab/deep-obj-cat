...
% (JLR) It's a little dense right at the beginning in this first paragraph. Maybe to start you can: 1) Introduce predictive learning as an alternative to supervised learning on hand-labeled datasets, 2) Breifly describe how your previous work shows how predictive learning can emerge from specific mechanisms in the thalamus/cortex, making sense of otherwise puzzling anatomical/physiological details, and then 3) describe the current contribution, investigating whether, when it's scaled up, the framework produces visual-object category learning unlike any existing machine-learning models. And then put the description of the biological details a little further down? 
...
% (JLR) remove "For example" in second sentence? It's not really an example of what's in the first sentence, it's kind of the whole thing, right?
% (JLR) "non-verbal primates, and pre-verbal human infants," -> I think removing those commas sounds better
...
% (JLR) Sentence starting "There were also saccadic eye movements" is a little long/difficult to parse. Can break into two: "There were also saccadic eye movements every other frame, introducing an additional predictive-learning challenge. An efferent copy signal enabled full prediction of the effects of the eye movement, and allows the model to capture predictive remapping (a widely-studied signature of predictive learning in the brain)\cite{DuhamelColbyGoldberg92,CavanaghHuntAfrazEtAl10}.
% (JLR) Missing "an" before "additional predictive-learning challenge" - or can be "challenges"?
% (JLR) Also, should "predictive remapping" be italicized or in quotes because it's a technical term?
% (JLR) "The only learning signal available to the model was the temporal difference prediction error between what it predicted to see in the next frame, compared to what was actually seen." -> "The only learning signal available to the model was a prediction error generated by the temporal difference between what it predicted to see in the next frame and what was actually seen."
...
% (JLR) Rewrite: "Furthermore, because this abstract category organization reflects the overall visual shapes of the objects as judged by human participants, this suggests that the model is extracting the geometrical shape information that is apparent once these objects are encoded in representations that are  invariant to differences in motion, rotation, and scaling present in the V1 visual inputs." -> "Furthermore, this abstract category organization reflects the overall visual shapes of the objects as judged by human participants, suggesting that the model is extracting geometrical shape information that is invariant to differences in motion, rotation, and scaling that are present in the V1 visual inputs."
...
% (JLR) I think I commented this last time, but Cadieu et al is not referenced using latex (twice). Is there no way to reference a citation in a way that types the author's name? e.g., with \citet{} or something? If not, then there should be a period in "et al." (twice)
...
% (JLR) Maybe would nice to really emphasize that these properties are not just "biologically motivated" but are really hallmarks of computation in the brain? "Biologically motivated" might understate it, and emphasizing their well-foundedness in neuroscience/biology might help sell the contribution of this work to biologists/neuroscientists? Also might help to emphasize that these properties are definitely not common in deep learning research. 

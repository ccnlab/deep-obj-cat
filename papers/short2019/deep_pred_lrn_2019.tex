...
% (JLR) Sounds a little odd with "learning" in there twice. I guess you don't want to use "predictive coding" to differentiate yourself from Friston/others? How about something like "Deep predictive learning as a model of human vision"? 
...
% (JLR) I don't think "seemingly-magic" should be hyphenated
% (JLR) "widely-held" shouldn't be hyphenated
% (JLR) How about: "Understanding the principles underlying the power of human learning is a widely held goal of research in machine learning, ..."
% (JLR) Are the big human-labeled datasets the issue we're addressing? Predictive learning doesn't require labels even if it's backprop right?
% (JLR) Maybe this was on purpose, but should there be no mention of vision or the visual system in the entire abstract/title?
...
% (JLR) "deep convolutional neural networks" = "DCNN", not "DCCN"?
% (JLR) Also, I've seen them abbreviated "CNN" more often than "DCNN".
...
% (JLR) I don't think "biologically-plausible" should be hyphenated. just read that in general adverbs ending in "ly" are not used in hyphenations: https://www.grammarbook.com/punctuation/hyphens.asp
% (JLR) Maybe italicize/emphasize "biologically" and "cognitively" second and third sentences to make that distinction clear
% (JLR) Might get some pushback on the point that CNNs all use labeled data - lot's don't, e.g. GANs, autoencoders, these video prediction networks, etc.
% (JLR) Should include citations for Rao/Ballard and Friston for predictive learning idea?
% (JLR) Wording: "In principle, all that this requires is for events to unfold over time, ..."
% (JLR) Also could number (1,2,3) the things in: "all that this requires is 1) ... 2) ... "?
...
% (JLR) "canonical circuits" rather than "replicated across the extent of these structures"?
% (JLR) Second sentence in paragraph above is a little long/hard to follow. Can 1) get rid of "furthermore", 2) End sentence after "invariant shape properties"?
% (JLR) I don't think PredNet should be considered state-of-the-art. It's strengths are that it tries to directly implement the predictive coding hypothesis in a deep learning setting, it replicates some biological data (they have an additional paper about that) and also there's another paper that uses it to model fMRI data. I think it generally performs pretty well, but is probably not the best overall in terms of the quality of its predictions.
% (JLR) Reword: "... support the idea that predictive learning is useful for shaping internal representations"?
% (JLR) "more strongly shaping internal representations" is a little vague. Maybe something like: "Comparison models ... demonstrate that predictive learning can support the development of abstract, categorical representations. However, these models do not learn much ..."
...
% (JLR) Didn't really introduce terms "superficial" and "deep". May be especially confusing given the different meaning of "deep" used to refer to the depth of the network as a whole (in the title, etc.)?
% (JLR) Maybe you should include a figure that incorporates some images of the laminar structure of cortex? Might be a good way to really emphasize the biological plausibility. Also seems important given that that is one of the key original proposals of the theory right?
% (JLR) Someone who's not reading closely might mistake (a) for actual inputs the model was trained/tested on. Having the real images right below is helpful, but I wonder if it would be better to just combine (a) and (c) somehow? I still think the poor quality of the predictions/reconstructions is too easy for the reader to misinterpret/overemphasize. Might be hard for someone to see how a network producing those blurry blobs could be representing abstract object-category information.
% (JLR) "superficial and deep networks" -> "superficial and deep layers"?
% (JLR) Usage of "DeepLeabra" and "WWI" is a little confusing - they're talking about the same model right? Or "DeepLeabra" refers more to the algorithm and "WWI" refers to the architecture? Also, is this the only mention of "DeepLeabra" in the whole paper?
% (JLR) Too much explanation of the model in the figure caption? e.g. I think the entire What*Where aspect is only mentioned here. Might be confusing for readers who skip the long captions...
% (JLR) Only Figure 1a is referenced in the main text. Can put a reference to 1b somewhere where you're introducing the model, and a reference to 1c where you're talking about the dataset
...
% (JLR) Should cite your 2014 paper in this paragraph?
% (JLR) The sentence that starts with "Consistent with the temporal-difference nature of biologically plausible error signals in this framework..." is a little unclear - is the main point here that the temporal organization of this alpha cycle allows predictive learning to *emerge*, i.e. as a natural consequence of a biologically plausible synaptic plasticity rule? Or is the point just to contrast with the explicit temporal difference representations in the Friston approach?
% (JLR) In general, in this paragraph it's a little hard to tease apart what is being cited as evidence and what is just a description of how the model works. Maybe you could just breifly talk about the actual findings first, and then proceed to describe your interpretation and how the model incorporates those findings?
% (JLR) Isn't it not exactly "mathematically equivalent" - and even if it's close enough, aren't the small discrepancies the key to the better performance of Leabra over the backprop version of the model? This sentence claiming mathematical equivalence might make it harder for people to understand the important differences later on.
% (JLR) "forms of predictive learning" -> "theories of predictive learning"?
% (JLR) I know there's not a lot of space for this, but if I was just reading this paper for computational insights into why the model performs better than the backprop version, there's not much to go on in this paragraph. Do you think more space should be devoted earlier on to explaining the aspects of the model that are important for the comparison results?
...
% (JLR) "higher-level, more abstract" -> "high-level, abstract"
% (JLR) First sentence is repetitive. Can replace first two with: "A critical question for predictive learning is whether it can be used to develop high-level, abstract representations that go beyond low-level visual structure, without using any additional inputs such as hand-labeled object categories."?
% (JLR) "...have not demonstrated this result" - might get pushback on this: 1) the prednet paper makes a point of demonstrating that you can decode some important latent variables in the videos, including steering angle in a driving dataset and the latent variables used to generate their synthetic face dataset, and 2) the point about "facilitating training" not being as relevant as "forming systematic categories de novo" is maybe not obvious. Since that's something that we're actually showing rather than intuiting, maybe rewrite to reflect that? Right now it kind of seems like a potentially controversial claim without any evidence. In general with this sentence you could just emphasize that no one has shown categorical representations emerge in a purely predictive learning setting?
...
% (JLR) adverb-hyphen thing in "externall-provided". Maybe just "external"?
% (JLR) I added a period in etc.
% (JLR) Would it worth noting that the saccades help to mitigate the absence of convolutions?
% (JLR) Not sure I understand what the "further predictive-learning challenge" is
...
% (JLR) it might be nice to have titles above each of the matrices so that you don't have to go digging into the long caption to find out what they are. Also, the labels aren't the labels too small? And I'm not sure the labels above the matrix add too much. Hopefully it should be obvious that the matrices are symmetric? 
...
% (TEH) COMMENT: If I understand, my sense is that the reader is likely to be thinking the human subjects may have been viewing the top row in Fig 1c instead of something more like row 2 -- maybe make more clear exactly what human subjects were viewing?
% (JLR) "at the level of the V1 primary visual input layer of the model" is a little wordy: "at the level of the V1 primary visual input" is enough?
% (JLR) Fourth sentence in this paragraph is pretty wordy. Alternative: "Thus the model, despite being trained only to generate accurate pixel-level predictions, has learned to represent these objects in an abstract way that goes beyond the raw pixel-level information."?
% (JLR) Fifth sentence starting "Furthermore, ..." is too long. Can start a new sentence after "looking at the same objects"
...
% (JLR) Cadieu is not cited using Latex?
...
% (TAH) COMMENT: Key claim - well-stated -- and well-emphasized throughout the paper! - SOCO!?
...
\begin{figure}
  ...
  % (TAH) COMMENT: I read item f. several times and have not been able to really understand the point being made.
  \label{fig.bpred}
\end{figure}
...
% (TAH) COMMENT: I'm a relative sophisticate in this domain and it wasn't immediately clear to me how Hebbian learning is similar to wt. decay - maybe a tad of explanation? "... by biasing learning towards ?.."
for predictive error-driven learning \cite{OReilly98,OReillyMunakata00}.

% (JLR) Almost always referring to DeepLeabra as the "biologically-based model" - aside from the hyphen issue, are you trying to avoid "DeepLeabra"? It's kind of a mouthful to say "biologically-based model" every time?
% (JLR) Add "additional" before "Hebbian form of learning" - reader might think that all of the learning in DeepLeabra is Hebbian, rather than a mixture of Hebbian/error-driven. And I guess the reference to the textbook covers exactly how that is implemented?
...
% (JLR) "feedforward backpropagation networks" - might want to stay consistent and just say "backpropagation"? Also it might be confusing to refer to them as networks, because in your experiments it's really a difference in learning algorithm?
% (JLR) "perhaps all of them" would be better in parentheses rather than with commas?
% (JLR) Do you think you need to spend the time to explain why you haven't yet done these experiments? Maybe you could just emphasize that all three are probably contributing?
% (JLR) And I'm still not sure I understand the point about the "fully recurrent backpropagation". Is it just that you'd have to backpropagate through a ton of time steps of the settling process? 
...
% (JLR) Might be better to use different colors for these plots to avoid confusion with the previous figures. Because in a sense they should both be red according to the color scheme of the previous ones (i.e. they're correlation wrt TE). 
...
% (TAH)COMMENT: One reading might imply individual token reps... BW?: "...more abstract, categorical object representations..."
...
%  (TAH) COMMENT: The below paragraph uses a lot of different meanings of the concept "deep" -- maybe to try to replace some uses with synonyms?
...
% (JAR) I like that paragraph ending sentence!!

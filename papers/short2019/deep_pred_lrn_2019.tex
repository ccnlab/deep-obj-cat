...
% (JLR) May want more citations after "questioned on biological grounds"? Aren't there are a ton of papers on that topic?
...
% (JLR) Isn't it not exactly "mathematically equivalent" - and even if it's close enough, aren't the small discrepancies the key  to the better performance of Leabra over the backprop version of the model?
...
suggesting that the model is learning about the same kind of overall geometrical shape information that is apparent once these objects are encoded in an invariant way that abstracts across the motion, rotation, and scaling transformations present in the V1 visual inputs.  We further verified that at the highest IT levels in the model, a consistent 
% (TEH) , spatially-invariant
representation is present across different views of the same object (e.g., the average correlation across frames within an object was .901), which is also evident in Figure~\ref{fig.rsa}a by virtue of the close similarity across multiple objects within the same category.
...
% (JAR) Formatting problem before this paragraph starts
% ROR: I'm not replicating it..?
...
% (TAH) COMMENT: It's probably formatting for Science, but these listing are kinda weird: no page numbers,and for @INCOLLECTION cases only one title is listed instead of two.
